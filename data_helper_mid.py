...
def get_data_info(folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    ...
    for folder in folder_list:
        ...
        fpaths = glob.glob('{}{}/*.JPEG'.format(par.image_dir, folder))
        imu = np.loadtxt('{}{}.csv'.format(par.imu_dir, folder),delimiter=',')
        ...
        if seq_len_range[0] == seq_len_range[1]:
            ...
            for st in start_frames:
                ...
                imu_segs = [imu[i:i + seq_len] for i in range(st, n_frames, jump)]
                Y += y_segs
                x_imu += imu_segs
                X_path += x_segs
    ...
    data = {'seq_len': X_len, 'image_path': X_path, 'pose': Y, 'imu': x_imu}
...
class ImageSequenceDataset(Dataset):
    def __init__(self, info_dataframe, resize_mode='crop', new_sizeize=None, img_mean=None, img_std=(1, 1, 1),
        ...
        if resize_mode == 'crop':
            ...
            transform_ops.append(albumentations.CentorCrop((new_sizeize[0].new_sizeize[1])))
    ...
    def __getitem__(self, index):
        ...
        groundtruth_sequence[1:] = groundtruth_sequence[1:] - groundtruth_sequence[
            0]  # get relative pose w.r.t. the first frame in the sequence
        ...
        imu_sequence = torch.FloatTensor(self.imu_arr[index])

        return (sequence_len, image_sequence, groundtruth_sequence, imu_sequence)
