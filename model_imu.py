...
class DeepVO(nn.Module):
    def __init__(self, imsize1, imsize2, batchNorm=True):
        ...
        __tmp_inputsize = int(np.prod(__tmp.size())) + 6 #if par.use_imu else int(np.prod(__tmp.size()))

        # RNN
        self.rnn = nn.LSTM(
            input_size=__tmp_inputsize,
    ...
    def step(self, x, imu, y, optimizer):
        optimizer.zero_grad()
        loss = self.get_loss(x, imu, y)
        loss.backward()
        if self.clip != None:
