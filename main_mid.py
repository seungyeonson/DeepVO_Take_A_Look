...
from model_imu import DeepVO
from data_helper_mid import get_data_info, SortedRandomBatchSampler, ImageSequenceDataset, get_partition_data_info
...
for ep in range(par.epochs):
	...
	gen = trange(len(train_dl))
	print("gen", gen)
	for i,(_, t_x, t_y, t_imu) in zip(gen,train_dl):
	...
	# for _, t_x, t_y in train_dl:
		...
		ls = M_deepvo.step(t_x, t_imu, t_y, optimizer).data.cpu().numpy()
	...
	gen = trange(len(valid_dl))
	print("gen", gen)
	for i,(_, v_x, v_y, v_imu) in zip(gen,valid_dl):
	...
	# for _, v_x, v_y in valid_dl:
		...
		v_ls = M_deepvo.get_loss(v_x,v_imu, v_y).data.cpu().numpy()
